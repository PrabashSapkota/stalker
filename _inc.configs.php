...
$APP_CONFIG['DEFAULT_ADMIN_PIN'] = "4321";
$APP_CONFIG['DEFAULT_STREAM_PROXY_STATUS'] = "OFF";
$APP_CONFIG['DATA_FOLDER'] = "__AppData__";
$APP_CONFIG['WHITELABEL_APP_DEVS'] = "@allinone_reborn";
...
function getRootBase($url)
{
    ...
    if(isset($xrl['host']) && !empty($xrl['host'])) {
        $output = $xrl['scheme']."://".$xrl['host'].":".$xrl['port'];
    }
    return $output;
}
...
function fixlogoissue($logo)
{
    ...
    if($custom_logo == 1) {
        $logo = $streamenvproto."://".$plhoth.str_replace(" ", "%20", str_replace(basename($_SERVER['PHP_SELF']), '', $_SERVER['PHP_SELF']))."assets/aio_tv_logo.png";
        return $logo;
    } else {
        return $logo;
    }
